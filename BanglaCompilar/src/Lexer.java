...
// The lexer reads raw source text and turns it into a stream of tokens.
public class Lexer {
    ...
    public List<Token> scanTokens() {
        ...
        // EOF marks the end of input so the parser knows when to stop.
    ...
    private void identifier() {
        // Bangla keywords and names are scanned the same way first.
    ...
    private void number() {
        // Digits keep extending the number token until something else appears.
        while (isBanglaDigit(peek())) advance();
        addToken(TokenType.NUMBER);
    }

    private void stringLiteral() {
        // Read until the closing quote, allowing new lines inside strings.
        while (!isAtEnd() && peek() != '"') {
            if (peek() == '\n') line++;
            advance();
        }
